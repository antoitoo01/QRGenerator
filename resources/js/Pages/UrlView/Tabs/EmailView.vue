...
const allTextInputAreValid = ref(false);

//Títulos botones:
const textButtonText = "INTRODUCE TEXTO";
...
watchEffect(() => {
  ...
  if (value.length > 0) {
    if (isValidEmail(value)) {
      ...
      if (subjectValue.value && msgValue.value && textValueIsValid.value) {
        allTextInputAreValid.value = true;
      } else {
        allTextInputAreValid.value = false;
      }
    } else {
      allTextInputAreValid.value = false;
    ...
    <div name="MainContainer" class="flex flex-col size-full pr-2">
      ...
      <div name="FirstButton">
        ...
        >
          ...
          <br />
          <p class="pb-2 mt-2"><b>Asunto</b></p>
          <input
          ...
          />
          <br />
          <p class="pb-2"><b>Mensaje</b></p>
          <textarea
            v-model="msgValue"
            class="custom-input max-h-44 min-h-11 h-11"
