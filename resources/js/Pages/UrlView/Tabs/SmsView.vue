<script setup>
import { ref, watch, watchEffect, computed, defineProps } from "vue";
...
const textValueIsValid = ref(false);
const allTextsAreValid = ref(false);

//Títulos botones:
const textButtonText = "INTRODUCE NÚMERO TELEFÓNICO";
...
const smsTo = computed(() => {
  return "SMSTO:" + textValue.value + ":" + smsBody.value;
});
...
function isValidPhoneNumber(text) {
  ...
  return phoneNumber.length >= 9 && phoneNumber.length <= 11;
}

watchEffect(() => {
  const value = textValue.value;
  if (isValidPhoneNumber(value)) {
    textValueIsValid.value = true;
    if (smsBody.value.length <= 0 || textValue.value.length <= 0) {
      allTextsAreValid.value = false;
    } else {
      allTextsAreValid.value = true;
    }
  } else {
    textValueIsValid.value = false;
    allTextsAreValid.value = false;
  }
});
  ...
  <form class="flex flex-row">
    ...
    <div name="MainContainer" class="flex flex-col size-full pr-2">
        ...
        <div name="FirstButton">
          ...
          >
            <p class="pb-2"><b>Tu número de teléfono</b></p>
            <input
              v-model="textValue"
              type="text"
              class="custom-input"
              placeholder="Introducir número telefónico"
            />
            <p
              class="text-slate-400 pt-2"
              v-show="!textValueIsValid && textValue.length > 0"
            >
              Tiene que ser un número de teléfono
            </p>

            <p class="pb-2 pt-4"><b>Mensaje del SMS</b></p>
            <textarea
              v-model="smsBody"
              class="custom-input min-h-11 max-h-44"
          ...
          >
            ...
            <div class="flex flex-row place-content-start overflow-auto">
              <div class="flex flex-col pr-14">
                ...
                >
                  <label for="transparent" class="mr-2"
      ...
      <div class="flex pl-2 w-full h-full">
        <div class="sm:flex md:fixed sm:overflow:auto pr-20">
          <div
